...
public class BoardForm : Form
{
    ...
    {
        Text          = "Checkers Game";
        ClientSize    = new Size(BoardLeft * 2 + BoardModel.Cols * CellSize + 220,
        ...
        _lblPlayer.Location  = new Point(cx, 10);
        _lblPlayer.Size      = new Size(190, 40);
        _lblPlayer.Font      = new Font("Segoe UI", 10, FontStyle.Bold);
        Controls.Add(_lblPlayer);

        _lblTimer.Location   = new Point(cx, 60);
        _lblTimer.Size       = new Size(190, 30);
        _lblTimer.Font       = new Font("Segoe UI", 12, FontStyle.Bold);
        _lblTimer.ForeColor  = Color.DarkRed;
        Controls.Add(_lblTimer);

        _lblStatus.Location  = new Point(cx, 100);
        _lblStatus.Size      = new Size(190, 60);
        _lblStatus.Font      = new Font("Segoe UI", 9);
        Controls.Add(_lblStatus);

        // Instruction label — tells the user how to use the free-draw feature
        var lblDraw = new Label
        {
            Text      = "Right-click + drag = draw\nLeft-click = move piece",
            Location  = new Point(cx, 168),
        ...
        _btnClear.Location   = new Point(cx, 210);
        _btnClear.Size       = new Size(190, 30);
        _btnClear.Text       = "Clear Drawing";
        _btnClear.Click     += (_, __) => { _drawGraphics.Clear(Color.Transparent); _boardPB.Invalidate(); };
        Controls.Add(_btnClear);

        _btnNewGame.Location = new Point(cx, 250);
        _btnNewGame.Size     = new Size(190, 30);
        _btnNewGame.Text     = "New Game";
        _btnNewGame.Click   += BtnNewGame_Click;
        Controls.Add(_btnNewGame);

        _btnReplay.Location  = new Point(cx, 290);
        _btnReplay.Size      = new Size(190, 30);
    ...
    private void WinTimer_Tick(object? sender, EventArgs e)
    {
        ...
        _winBlinkCount++;
        _boardPB.Invalidate();
        if (_winBlinkCount >= 34)  // ~5 seconds at 300ms intervals = ~16 toggles (34 ticks)
        {
            _winTimer.Stop();
            _winBlink   = false;
            _winBlinkOn = false;
    ...
    private void BoardPB_Paint(object? sender, PaintEventArgs e)
    {
        ...
        for (int r = 0; r < BoardModel.Rows; r++)
        {
            for (int c = 0; c < BoardModel.Cols; c++)
            {
                ...
                {
                    Color pieceColor;
                    if (_winBlink && _winBlinkOn && piece == _winnerSide)
                        pieceColor = Color.LimeGreen;  // blink green for winner
                    else
                        pieceColor = piece == Side.Human ? Color.Red : Color.DodgerBlue;
